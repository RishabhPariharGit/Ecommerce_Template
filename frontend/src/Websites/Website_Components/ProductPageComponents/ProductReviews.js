    ...
    const ProductReviews = [
        {
          ...
          title: "I absolutely love this hoodie! The material is so soft and cozy, perfect for chilly mornings or lounging at home. The fit is spot-on, and the design gets me compliments everywhere I go. ",
        },
        {
        ...
        <Slider {...settings}>
          {ProductReviews.map((ProductReview, index) => (
            <div key={index} className="slider-item">
              <div className="icon">{ProductReview.icon}</div>
              <div className="title">{ProductReview.title}</div>
            </div>
          ))}
        </Slider>
      ) : (
